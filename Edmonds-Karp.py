...
def EdmondsKarp(adjMatrix, capMatrix, source, sink):
    n = len(capMatrix)
    ...
    while True:
        ...
        pathFlow, path = BFS(adjMatrix, capMatrix, source, sink, residualMatrix, path, M)
...
def BFS(adjMatrix, capMatrix, source, sink, residualMatrix, path, M):
...
if __name__ == "__main__":
    ...
    for i in range(100):
        EdmondsKarp(adjMatrix, capMatrix, source, sink)
   
    end = datetime.datetime.now()

    print("Max flow with BFS: ", EdmondsKarp(adjMatrix, capMatrix, source, sink))
